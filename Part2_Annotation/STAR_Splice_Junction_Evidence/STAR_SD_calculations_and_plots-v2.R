...
for (gene in genes){
	...
	for(i in 9:ncol(gene_totalCPM)){
		...
		if (i == 9){
			plot(as.numeric(junction.names), plot_CPM,
					pch=16, main="TOTAL SJ Count Per Million (CPM)",
	...
	if(gene == "RJF.Contig1.LOC30.MHCY8a"){
		write.table(log_table.above_linear_10,"SD_MHCY8_min10_export.txt", quote=F, sep="\t", row.names=F)
	}#end if(gene == "RJF.Contig1.LOC30.MHCY8a")
