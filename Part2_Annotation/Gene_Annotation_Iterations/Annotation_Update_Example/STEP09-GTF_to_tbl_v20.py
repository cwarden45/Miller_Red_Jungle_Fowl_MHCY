...
tbl = contig+"_BLAST_annotated_gene_v22.tbl"
...
while line:
	...
	if lineCount > 1:
		...
		if len(copyArr) == 1:		
			...
			temp_gene_type = re.sub("\d+$","",copyArr[0])
			#print temp_gene_type
			copyArr_num = copyArr
			for i in xrange(0,len(copyArr)):
				copyArr_num[i] = int(re.sub(temp_gene_type,"",copyArr[i]))
			#print copyArr_num
			copyArr_num.sort()
			for i in xrange(0,len(copyArr)):
				copyArr[i] = temp_gene_type + str(copyArr_num[i])
	...
	if (type == "misc_RNA"):
		...
		if (type != "rRNA"):
			...
			if ID in typeHash:
				...
				sys.exit()
			description = re.sub(" \(rRNA\)","",description)
