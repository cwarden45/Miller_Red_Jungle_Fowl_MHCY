...
contig_nums = 1:4
STAR_category_file = "combined_splice_junctions-STAR-v4-CATEGORIES-param9-ADJUSTED_CPM.txt"
...
for (i in 1:length(rRNA_LOC)){
	...
	for (j in 1:nrow(temp.table)){
		...
		if (j == 1){
			rRNA_start = loc_arr[1]
			rRNA_stop = loc_arr[2]
		}else{
			rRNA_start = min(loc_arr[1], rRNA_start)
			rRNA_stop = max(loc_arr[2], rRNA_stop)
		}
